...
function highlight(text, q) {
  ...
  const str = String(text); // number bhi ho sakta hai — pehle string banao
  const idx = str.toLowerCase().indexOf(q.toLowerCase());
  if (idx === -1) return str;
  return str.slice(0, idx)
    + `<mark style="background:#fff3cd; border-radius:2px; padding:0 1px;">${str.slice(idx, idx + q.length)}</mark>`
