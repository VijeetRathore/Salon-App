...
function pushRecords(storeName, records) {
  ...
  // Naye records track karo (notification sirf inke liye)
  const newBills = [];
  ...
  records.forEach((record) => {
    ...
    // Sirf naye bills track karo — updates pe notification nahi
    if (isNew && storeName === 'bills') newBills.push(record);
  });

  logSync(storeName, records.length);

  // Notification sirf NEW bill insertions pe — updates (markSent, etc.) pe nahi
