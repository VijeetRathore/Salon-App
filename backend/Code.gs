...
function pushRecords(storeName, records) {
  ...
  // Naye records aur sent updates track karo
  const newBills  = [];
  const sentBills = [];
  ...
  records.forEach((record) => {
    ...
    if (storeName === 'bills') {
      if (isNew) newBills.push(record);
      // Bill update: whatsappStatus sent hua → creator ko notify karo
      else if (String(flat.whatsappStatus) === 'sent' && flat.createdByUserId) {
        sentBills.push(record);
      }
    }
  });
  ...
  // 1. Naye bills → owners ko notify karo
  if (newBills.length > 0) {
    try {
      const total = newBills.reduce((s, r) => s + (Number(r.total) || 0), 0);
      const hasWA = newBills.some(r => r.whatsappStatus === 'pending');
      const title = newBills.length === 1
        ? `New Bill — ₹${total.toLocaleString('en-IN')}`
        : `${newBills.length} Bills — ₹${total.toLocaleString('en-IN')}`;
      const body  = hasWA ? 'WhatsApp message ready — queue mein bhejo' : 'Payment received';
      const url   = hasWA ? './whatsapp-queue.html' : './billing.html';
      notifyOwners('bill', title, body, url);
    } catch (e) { }
  }

  // 2. Bill sent → bill banane wale user ko confirm karo (owners ko nahi)
  for (const bill of sentBills) {
    try {
      const flat      = flattenRecord(bill);
      const customer  = String(flat.customerName || flat.customerId || 'Customer');
      notifyUser(String(flat.createdByUserId), 'bill-sent',
        'Bill Sent ✅',
        customer + ' ko WhatsApp message bhej diya gaya',
        './whatsapp-queue.html');
    } catch (e) { }
  }
...
function sendFCMMessage(accessToken, fcmToken, title, body, type, targetUrl) {
  ...
  // IMPORTANT: notification field bilkul mat bhejo — agar notification field hogi to
  // Firebase khud ek notification dikhayega (default, bina click handler ke) AUR
  // hamare onBackgroundMessage se bhi ek dikhayega = 2 notifications.
  // Sirf data field bhejo — hamare SW ka onBackgroundMessage ek hi notification dikhayega.
  const payload = {
    token: fcmToken,
    data: {
      type:  type        || 'general',
    ...
    // Android/web ko high priority dena zaroori hai data-only messages ke liye
...
// Specific user ko notify karo (userId se)
function notifyUser(userId, type, title, body, url) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][userIdIdx] || '').trim() !== userId.trim()) continue;
    const isActive = values[i][activeIdx];
    if (isActive === false || String(isActive).toUpperCase() === 'FALSE') return;

    let tokens = [];
    try { tokens = JSON.parse(values[i][tokIdx] || '[]'); } catch { tokens = []; }

    let accessToken = null;
    for (const token of tokens) {
      if (!token) continue;
      if (!accessToken) accessToken = getFCMAccessToken();
      try { sendFCMMessage(accessToken, token, title, body, type, url); } catch (e) { }
    }
    return;
  }
}
