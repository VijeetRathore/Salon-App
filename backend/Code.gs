...
function pushRecords(storeName, records) {
  ...
  // WhatsApp queue → owner devices ko notify karo
  if (storeName === 'pendingMessages') {
    const pending = records.filter(r => r.status === 'pending');
    if (pending.length) {
      try {
        notifyOwners('whatsapp', 'New WhatsApp Message',
          `${pending.length} message(s) waiting in Queue`, './whatsapp-queue.html');
      } catch (e) { }
    }
  }

  // Nayi bill → owner devices ko notify karo
  if (storeName === 'bills' && records.length > 0) {
    try {
      const total = records.reduce((s, r) => s + (Number(r.total) || 0), 0);
      notifyOwners('bill',
        records.length === 1 ? 'New Bill Created' : `${records.length} Bills Created`,
